...
struct PCA9541Device {
    ...
    enum pca9541_other_master_mode om_mode;
    int control_reads; /* Number of control reads done in this test. */
};
...
static uint8_t pca9541_next_reg(PCA9541Device *pca9541, int offset)
{
    uint8_t reg = pca9541->curr_regnum & 0x3;
...
static void pca9541_write_next(PCA9541Device *pca9541, uint8_t val, int offset)
{
    uint8_t reg = pca9541_next_reg(pca9541, offset);
    uint8_t newval;
...
static void pca9541_write_data(SMBusDevice *dev, uint8_t cmd,
                               uint8_t *buf, int len)
{
    PCA9541Device *pca9541 = PCA9541(dev);
    int i;

    pca9541->curr_regnum = cmd;
    for (i = 0; i < len; i++) {
        pca9541_write_next(pca9541, buf[i], i);
    }
}

static uint8_t pca9541_read_next(PCA9541Device *pca9541, int offset)
{
    uint8_t reg = pca9541_next_reg(pca9541, offset);
    uint8_t val;
...
static uint8_t pca9541_read_data(SMBusDevice *dev, uint8_t cmd, int n)
{
    PCA9541Device *pca9541 = PCA9541(dev);

    pca9541->curr_regnum = cmd;
    return pca9541_read_next(pca9541, n);
}
...
static void pca9541_child_added(BusState *bus, DeviceState *dev)
{
    ...
    if (!master) {
        master = PCA9541_MASTER(i2c_create_slave(pca9541->master,
                                                 TYPE_PCA9541_MASTER, addr));
...
static int pca9541_init(SMBusDevice *smbdev)
{
...
static int pca9541_master_device_init(I2CSlave *i2c)
{
    return 0;
}
...
static int pca9541_master_recv(I2CSlave *s)
{
    PCA9541Master *master = PCA9541_MASTER(s);
    I2CSlave *slave = master->slave;
    int rv = -1;
...
static void pca9541_master_class_initfn(ObjectClass *klass, void *data)
{
    ...
    sc->init = pca9541_master_device_init;
    sc->event_check = pca9541_master_event;
...
static void pca9541_class_initfn(ObjectClass *oc, void *data)
{
    ...
    sc->init = pca9541_init;
    sc->write_data = pca9541_write_data;
    sc->read_data = pca9541_read_data;
    dc->props = pca9541_properties;
}
